...
impl AgentConfig {
    pub fn load() -> anyhow::Result<Self> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                "--server" => { i += 1; if i < args.len() { cfg.server_url = args[i].clone(); } }
                "--key" => { i += 1; if i < args.len() { cfg.ingest_key = args[i].clone(); } }
                "--service" => { i += 1; if i < args.len() { cfg.service = args[i].clone(); } }
                "--file" => { i += 1; if i < args.len() { cfg.file = Some(args[i].clone()); } }
                "--interval" => { i += 1; if i < args.len() { cfg.interval_secs = args[i].parse().unwrap_or(cfg.interval_secs); } }
                "--once" => cfg.once = true,
                "--help" | "-h" => {
