...
async fn main() -> anyhow::Result<()> {
    ...
    let server = take_opt(&mut args, "--server").unwrap_or_else(|| "http://127.0.0.1:8092".into());
    ...
    match cmd.as_str() {
        "produce" => {
            ...
            let count: usize = take_opt(&mut args, "--count")
                .unwrap_or_else(|| "1".into())
                .parse()?;
            let partition = take_opt(&mut args, "--partition")
                .map(|v| v.parse::<u32>())
                .transpose()?;
            let value = take_opt(&mut args, "--value")
                .unwrap_or_else(|| "hello from infinity-stream-cli".into());
            for i in 0..count {
                let body = json!({"partition": partition, "key": format!("cli-{i}"), "value": {"message": value, "n": i}});
                let res = http
                    .post(format!("{server}/v1/topics/{topic}/produce"))
                    .bearer_auth(&key)
                    .json(&body)
                    .send()
                    .await?;
        ...
        "consume" => {
            let topic = pop(&mut args, "topic")?;
            let partition: u32 = take_opt(&mut args, "--partition")
                .unwrap_or_else(|| "0".into())
                .parse()?;
            let offset: u64 = take_opt(&mut args, "--offset")
                .unwrap_or_else(|| "0".into())
                .parse()?;
            let max: usize = take_opt(&mut args, "--max")
        ...
        "search" => {
            ...
            let query = take_opt(&mut args, "--query")
                .or_else(|| take_opt(&mut args, "-q"))
                .context("--query required")?;
            let k: usize = take_opt(&mut args, "--k")
                .unwrap_or_else(|| "10".into())
                .parse()?;
            let res = http
                .get(format!("{server}/v1/indexes/{index}/search"))
                .bearer_auth(&key)
