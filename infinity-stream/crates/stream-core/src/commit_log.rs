...
impl CommitLog {
    pub fn open(data_dir: impl AsRef<Path>, segment_max_bytes: u64) -> Result<Self> {
        ...
        let mut me = Self { root, segment_max_bytes: segment_max_bytes.max(256), partitions: HashMap::new() };
        me.recover()?;
        Ok(me)
    }
    ...
    pub fn create_partition(&mut self, topic: &str, partition: u32) -> Result<()> {
        ...
        self.partitions.entry((topic.to_string(), partition)).or_default();
        Ok(())
    }
    ...
    pub fn append(&mut self, topic: &str, partition: u32, records: Vec<NewRecord>) -> Result<Vec<LogRecord>> {
        ...
        for rec in records {
            ...
            let stored = StoredRecord { offset, key: rec.key, value: rec.value, timestamp: Utc::now().to_rfc3339() };
            ...
            part.next_offset += 1;
            out.push(LogRecord {
                topic: topic.to_string(), partition, offset,
                key: stored.key, value: stored.value, timestamp: stored.timestamp,
            });
        }
        Ok(out)
    }

    pub fn read_from(&self, topic: &str, partition: u32, offset: u64, max: usize) -> Result<Vec<LogRecord>> {
        let Some(part) = self.partitions.get(&(topic.to_string(), partition)) else { return Ok(vec![]); };
        let mut out = Vec::new();
        let mut cur = offset;
        while out.len() < max {
            let Some(pos) = part.index.get(&cur) else { break; };
            ...
            out.push(LogRecord { topic: topic.to_string(), partition, offset: stored.offset, key: stored.key, value: stored.value, timestamp: stored.timestamp });
            cur += 1;
        }
        Ok(out)
    }
    ...
    pub fn next_offset(&self, topic: &str, partition: u32) -> u64 {
        self.partitions.get(&(topic.to_string(), partition)).map(|p| p.next_offset).unwrap_or(0)
    }
    ...
    fn recover(&mut self) -> Result<()> {
        ...
        for topic_entry in fs::read_dir(&self.root)? {
            ...
            if !topic_entry.file_type()?.is_dir() { continue; }
            ...
            for part_entry in fs::read_dir(topic_entry.path())? {
                ...
                if !part_entry.file_type()?.is_dir() { continue; }
                let partition: u32 = part_entry.file_name().to_string_lossy().parse().unwrap_or(0);
    ...
    fn recover_partition(&mut self, topic: &str, partition: u32, dir: &Path) -> Result<()> {
        ...
        for path in files {
            let base = path.file_stem().and_then(|s| s.to_str()).and_then(|s| s.parse::<u64>().ok()).unwrap_or(0);
            ...
            loop {
                ...
                if !trimmed.is_empty() {
                    let stored: StoredRecord = serde_json::from_str(trimmed).map_err(|e| CoreError::Invalid(format!("corrupt segment {}: {e}", path.display())))?;
                    part.index.insert(stored.offset, OffsetPos { segment: path.clone(), pos });
...
mod tests {
    ...
    fn test_dir(name: &str) -> PathBuf {
        ...
        std::env::current_dir().unwrap().join("target").join("stream-core-tests").join(format!("{name}-{n}"))
    }
    ...
    fn append_and_read() {
        ...
        let written = log.append("orders", 0, vec![NewRecord { key: Some("k".into()), value: json!({"n":1}) }]).unwrap();
    ...
    fn segment_rolls() {
        ...
        for i in 0..10 {
            log.append("t", 0, vec![NewRecord { key: None, value: json!({"payload":"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", "i": i}) }]).unwrap();
        }
    ...
    fn recovery_rebuilds_offsets() {
        let dir = test_dir("recovery");
        {
            ...
            log.append("events", 1, vec![NewRecord { key: None, value: json!("a") }, NewRecord { key: None, value: json!("b") }]).unwrap();
        }
        ...
        let appended = recovered.append("events", 1, vec![NewRecord { key: None, value: json!("c") }]).unwrap();
