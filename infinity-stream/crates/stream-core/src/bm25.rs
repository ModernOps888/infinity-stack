...
impl Bm25Index {
    ...
    pub fn search(&self, query: &str, k: usize) -> Vec<ScoredDoc> {
        ...
        if q_terms.is_empty() || self.docs.is_empty() || k == 0 { return vec![]; }
        ...
        let mut ranked: Vec<ScoredDoc> = scores.into_iter().map(|(doc_id, score)| ScoredDoc { doc_id, score }).collect();
...
fn fields_to_text(fields: &HashMap<String, Value>) -> String {
    fn push_value(out: &mut String, v: &Value) {
        match v {
            Value::String(s) => { out.push(' '); out.push_str(s); }
            Value::Number(n) => { out.push(' '); out.push_str(&n.to_string()); }
            Value::Bool(b) => { out.push(' '); out.push_str(if *b { "true" } else { "false" }); }
            Value::Array(a) => for v in a { push_value(out, v); },
            Value::Object(o) => for v in o.values() { push_value(out, v); },
            Value::Null => {}
        }
    }
    let mut out = String::new();
    for (k, v) in fields {
        out.push(' '); out.push_str(k); push_value(&mut out, v);
    }
    out
}
...
const STOPWORDS: &[&str] = &[
    ...
    "of", "on", "that", "the", "to", "was", "were", "will", "with", "or", "this", "these", "those",
];
...
mod tests {
    ...
    fn relevant_docs_rank_above_irrelevant() {
        ...
        idx.index("rust", doc("Rust streaming", "append only logs and fast search"));
        idx.index("cake", doc("Cake recipe", "flour sugar butter"));
        idx.index("logs", doc("Streaming logs", "durable commit logs search search"));
        let hits = idx.search("streaming commit logs", 3);
        assert_eq!(hits[0].doc_id, "logs");
        assert!(hits.iter().position(|h| h.doc_id == "logs").unwrap() < hits.iter().position(|h| h.doc_id == "cake").unwrap_or(usize::MAX));
    }
}
